...
def generate_recommendations(CURRENT_USER, num_recommendations):
    ...
    for movie in movies:
        if possibility_index(CURRENT_USER, movie.id_get()) < lowest_possibility:
            lowest_possibility = possibility_index(CURRENT_USER, movie.id_get())
    print("LOWEST", lowest_possibility)
    ...
    print("HIGHEST", highest_possibility)
        ...
        if counter < len(recommended_movies):
            for movie in movies:
                if movie.id_get() == movie.id:
                    print(CURRENT_USER, movie.id_get())
                    print(possibility_index(CURRENT_USER, movie), "-", lowest_possibility)
                    print("GENERATE:", movie.title, movie.year, "POSSIBILITY: %{}".format(possibility_index(CURRENT_USER, (possibility_index(CURRENT_USER, movie) - lowest_possibility) / (highest_possibility - lowest_possibility))), "\n")
                    counter += 1
